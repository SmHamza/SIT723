...
# origianl timesteps = 100000, changed to 10000


def train_GAIL(env_train, model_name, timesteps=50000):
    """GAIL Model"""
    # from stable_baselines.gail import ExportDataset, generate_expert_traj
    start = time.time()
    # generate expert trajectories, origial n_timesteps = 100000 changed to 100
    model = SAC('MlpPolicy', env_train, verbose=1)
    generate_expert_traj(model, 'expert_model_gail',
                         n_timesteps=100000, n_episodes=10)
...
def run_ensemble_strategy(df, unique_trade_date, rebalance_window, validation_window) -> None:
    ...
    for i in range(rebalance_window + validation_window, len(unique_trade_date), rebalance_window):
        ...
        print("======TD3 Training========")
        model_td3 = train_TD3(
            env_train, model_name="TD3_10k_dow_{}".format(i), timesteps=10000)
        ...
        print("======GAIL Training========")
        model_gail = train_GAIL(
            env_train, model_name="GAIL_30k_dow_{}".format(i), timesteps=1)  # change timesteps
        print("======GAIL Validation from: ", unique_trade_date[i - rebalance_window - validation_window], "to ",
              unique_trade_date[i - rebalance_window])
        DRL_validation(model=model_gail, test_data=validation,
                       test_env=env_val, test_obs=obs_val)
        ...
        if (sharpe_ppo >= sharpe_a2c) & (sharpe_ppo >= sharpe_ddpg) & (sharpe_ppo >= sharpe_gail) & (sharpe_ppo > sharpe_td3):
            model_ensemble = model_ppo
            model_use.append('PPO')
        elif (sharpe_a2c > sharpe_ppo) & (sharpe_a2c > sharpe_ddpg) & (sharpe_a2c > sharpe_gail) & (sharpe_a2c > sharpe_td3):
            model_ensemble = model_a2c
            model_use.append('A2C')
        elif (sharpe_td3 > sharpe_ppo) & (sharpe_td3 > sharpe_ddpg) & (sharpe_td3 > sharpe_a2c) & (sharpe_td3 > sharpe_gail):
            model_ensemble = model_td3
            model_use.append('TD3')
        elif (sharpe_gail > sharpe_ppo) & (sharpe_gail > sharpe_ddpg) & (sharpe_gail > sharpe_a2c) & (sharpe_gail > sharpe_td3):
